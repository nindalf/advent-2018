...
#[allow(dead_code)]
fn compute_dupe_freq() -> i32 {
    let mut index = 0;
    let mut sum = 0;
    let len = input.len();
    let mut set: HashSet<i32> = HashSet::new();
    loop {
        sum += input.get(index % len).unwrap();
        index += 1;
        if set.contains(&sum) {
            break;
        }
...
mod tests {
    #[test]
    fn test_compute() {
        println!("{}", super::compute_sum());
        println!("{}", super::compute_dupe_freq());
    }
}
...
impl FromStr for Sky {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let stars: Vec<Star> = s
            .lines()
            .map(|line| Star::from_str(line).unwrap())
            .collect();
        let mut sky = Sky {
            stars,
            length: 0,
...
impl Sky {
    fn position(&self, star: &Star) -> usize {
        ((star.x + self.offset_x) + (star.y + self.offset_y) * (self.length + 1)) as usize
    }

    fn update_positions(&mut self, seconds: u64) {
        for _ in 0..seconds {
    ...
    fn reverse(&mut self, seconds: u64) {
        for _ in 0..seconds {
            for star in self.stars.iter_mut() {
                star.x -= star.v_x;
                star.y -= star.v_y;
    ...
    fn calc_bounds(&mut self) {
        let max_x = self.stars.iter().map(|star| star.x).max().unwrap();
        let min_x = self.stars.iter().map(|star| star.x).min().unwrap();

        let max_y = self.stars.iter().map(|star| star.y).max().unwrap();
        let min_y = self.stars.iter().map(|star| star.y).min().unwrap();
    ...
    #[allow(dead_code)]
    fn wait_for_message(&mut self) -> u64 {
        let mut box_size = self.length * self.breadth;
        let mut iterations = 0;
        loop {
            self.update_positions(1);
            self.calc_bounds();
            let new_size = self.length * self.breadth;
            if new_size > box_size {
                self.reverse(1);
                self.calc_bounds();
                return iterations;
            }
...
impl Display for Sky {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        self.stars
            .iter()
            .map(|star| self.position(star))
            .for_each(|position| grid[position] = '#');
        let result: String = grid.iter().collect();
        write!(f, "{}", result)
    }
}
...
impl FromStr for Star {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        lazy_static! {
            static ref RE: Regex = Regex::new(r"position=<(?P<x>[\- 0-9]*), (?P<y>[\- 0-9]*)> velocity=<(?P<v_x>[\- 0-9]*), (?P<v_y>[\- 0-9]*)>").unwrap();
        }

        let caps = match RE.captures(s) {
            None => {
                println!("{}", s);
                return err!("unrecognized position/velocity - ");
            }
            Some(caps) => caps,
        };

        let x: i64 = caps["x"].trim().parse()?;
        let y: i64 = caps["y"].trim().parse()?;
        let v_x: i64 = caps["v_x"].trim().parse()?;
        let v_y: i64 = caps["v_y"].trim().parse()?;
...
mod tests {
    ...
    #[test]
    fn test_sky() {
        let sky: Sky = TEST_INPUT.parse().unwrap();
        assert_eq!(22, sky.length);
        assert_eq!(16, sky.breadth);
        assert_eq!(31, sky.stars.len());
        assert_eq!(TEST_SKY, sky.to_string());
    }

    #[test]
    fn test_real_message() {
        let mut sky: Sky = TEST_INPUT.parse().unwrap();
        sky.wait_for_message();
    ...
    fn test_optimal_iterations() {
        let mut sky: Sky = TEST_INPUT.parse().unwrap();
        assert_eq!(3, sky.wait_for_message());
        assert_eq!(TEST_MESSAGE, sky.to_string());

        let mut sky: Sky = REAL_INPUT.parse().unwrap();
        assert_eq!(10558, sky.wait_for_message());
